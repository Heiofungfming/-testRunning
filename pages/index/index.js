...
Page({
  ...
  keepPaint: function() {
    ...
    if (total_micro_second % 3000 == 0) { //1000为1s
      //that.getLocation();
      utils.getLocation().then((res) => {
        that.setData({
        ...
        that.setData({
          meters: Math.round(Number(that.data.meters) + dis) 
        })
      })
  ...
  randomPoi: function() {
    ...
    for (var i = 0; i < n; i++) {
      //随机到圆心的距离
      var temp_r = this.getRandom(r * 2 / 3, r * 5 / 6);
      ...
      //计算出纬度
      latitudes[i] = la + temp_y / (111319.5555 * 2.0);
      //计算出经度
      longitudes[i] = lo + temp_x / (111319.5555 * Math.cos(la) * 3.0);
    }
